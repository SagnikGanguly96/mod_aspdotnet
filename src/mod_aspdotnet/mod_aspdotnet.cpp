...
#include "../ApacheWeb/ApacheWebVersion.h"
...
static HRESULT init_clr4_runtime(asp_net_conf_t* globalconf, ICLRMetaHost* pMetaHost) {
	...
	do {
		...
		init_runtime_version(globalconf, const_cast<wchar_t*>(runtimeVersion));
	} while(false);
...
static HRESULT init_clr2_runtime(asp_net_conf_t* globalconf) {
	...
	init_runtime_version(globalconf, const_cast<wchar_t*>(pszVersion));
...
static HRESULT init_asp_engine(server_rec* global_server) {
	...
	apr_pool_cleanup_register(global_conf->pool, NULL, asp_net_stop, apr_pool_cleanup_null);

	ap_log_error(APLOG_MARK, APLOG_DEBUG | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: Module initialization commencing...");

	HRESULT hr = init_clr_runtime(global_conf);

	ap_log_error(APLOG_MARK, APLOG_NOTICE | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: It has loaded version %s of the .NET CLR engine.", global_conf->cor_version);

	hr = global_conf->pCorRuntime->Start();
	if(FAILED(hr)) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not start the .NET CLR engine.");
		_com_raise_error(hr);
	}

	ap_log_error(APLOG_MARK, APLOG_DEBUG | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: Module started .NET CLR...");

	IUnknown* pAppDomainIUnk = NULL;
	hr = global_conf->pCorRuntime->GetDefaultDomain(&pAppDomainIUnk);
	if(FAILED(hr)) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not retrieve the .NET default application domain.");
		_com_raise_error(hr);
	}
	if(!pAppDomainIUnk) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not retrieve the .NET default application domain's interface.");
		_com_raise_error(E_NOINTERFACE);
	}

	_AppDomain* pDefaultDomain = NULL;
	hr = pAppDomainIUnk->QueryInterface(__uuidof(_AppDomain), (void**)&pDefaultDomain);
	// Done with pAppDomainIUnk
	pAppDomainIUnk->Release();

	if(FAILED(hr)) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not retrieve the .NET default application domain's _AppDomain interface.");
		_com_raise_error(hr);
	}
	if(!pDefaultDomain) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not retrieve the .NET default application domain _AppDomain interface.");
		_com_raise_error(E_NOINTERFACE);
	}

	ap_log_error(APLOG_MARK, APLOG_DEBUG | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: Module initialized .NET default AppDomain...");

	_ObjectHandle* pObjHandle = NULL;
	hr = pDefaultDomain->CreateInstance(_bstr_t(
		"Apache.Web, "
	...
	if(FAILED(hr)) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not create the .NET interface for the Apache.Web.HostFactory.");
		_com_raise_error(hr);
	}
	if(!pObjHandle) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not retrieve the .NET object for the Apache.Web.HostFactory.");
		_com_raise_error(E_NOINTERFACE);
	}
	...
	if(FAILED(hr)) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not unwrap the COM interface for the Apache.Web.HostFactory.");
		_com_raise_error(hr);
	}
	if(!vHostFactory.pdispVal) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not retrieve the unknown COM interface for the Apache.Web.HostFactory.");
		_com_raise_error(E_NOINTERFACE);
	}

	hr = vHostFactory.pdispVal->QueryInterface(__uuidof(IApacheWebHostFactory), (void**)&global_conf->pHostFactory);
	// Done with vHostFactory
	VariantClear(&vHostFactory);

	if(FAILED(hr)) {
		ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Could not retrieve the COM interface for the Apache.Web.HostFactory.");
		_com_raise_error(hr);
	}

	ap_log_error(APLOG_MARK, APLOG_DEBUG | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: Module initialized HostFactory...");
	...
	if(FAILED(hr)) {
		ap_log_error(
			APLOG_MARK,
			APLOG_ERR,
			APR_FROM_OS_ERROR(hr),
			global_server,
			"mod_aspdotnet: Could not correctly configure the Apache.Web.HostFactory."
		);
		_com_raise_error(hr);
	}

	ap_log_error(APLOG_MARK, APLOG_DEBUG | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: HostFactory configuration complete.");
...
static int asp_net_handler(request_rec* r) {
	...
	if(!r->uri || (r->uri[0] != '/')) {
		ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: Forbidden, request URI was not absolute: %s", r->uri ? r->uri : "<NULL>");
		return HTTP_FORBIDDEN;
	}

	asp_net_dirconf_t* dirconf = (asp_net_dirconf_t*)ap_get_module_config(r->per_dir_config, &aspdotnet_module);

	if(r->finfo.filetype == APR_REG) {
		if(!(dirconf->enable & ASPNET_FILES)) {
			ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: Forbidden, 'AspNet Files' is not enabled for: %s (file %s)", r->uri, r->filename);
			return HTTP_FORBIDDEN;
		}
	} else if(r->finfo.filetype == APR_DIR) {
		if(!(dirconf->enable & ASPNET_DIRS)) {
			ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: Forbidden, 'AspNet Dirs' is not enabled for: %s (directory %s)", r->uri, r->filename);
			return HTTP_FORBIDDEN;
		}
	} else if(!r->finfo.filetype) {
		if(!(dirconf->enable & ASPNET_VIRTUAL)) {
			ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: Forbidden, 'AspNet Virtual' is not enabled for: %s", r->uri);
			if(dirconf->enable & (ASPNET_FILES | ASPNET_DIRS)) {
				ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: File not found or unable to stat: %s", r->filename);
			}
			return HTTP_NOT_FOUND;
		}
	} else {
		ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: Invalid file type for: %s (it is not a recognized file or directory)", r->filename);
		return HTTP_NOT_FOUND;
	}
	...
	if((r->used_path_info == AP_REQ_REJECT_PATH_INFO) &&
		...
		/* default to accept */
		ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: AcceptPathInfo off for %s disallows user's path %s", r->filename, r->path_info);
		return HTTP_NOT_FOUND;
	}

	asp_net_srvconf_t* srvconf = (asp_net_srvconf_t*)ap_get_module_config(r->server->module_config, &aspdotnet_module);
	...
	if(!item || !host) {
		ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: No AspNetMount URI for request: %s", r->uri);
	...
	if(FAILED(hr)) {
		ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), r, "mod_aspdotnet: CoInitialize failed!");
		return HTTP_INTERNAL_SERVER_ERROR;
	}
	...
	try {
		status = global_conf->pHostFactory->HandleHostRequest(host->host_key, (UINT_PTR)r);
	} catch(_com_error err) {
		ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(err.Error()), r, "mod_aspdotnet: CreateRequest %s processing failed, file %s", r->uri, r->filename);
		char* desc = (char*)err.Description();
		if(desc) {
			ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(err.Error()), r, "mod_aspdotnet: %s", desc);
		}
		status = HTTP_INTERNAL_SERVER_ERROR;
	} catch(HRESULT hr) {
		ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), r, "mod_aspdotnet: CreateRequest %s processing failed, file %s", r->uri, r->filename);
		status = HTTP_INTERNAL_SERVER_ERROR;
	} catch(...) {
		ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, r, "mod_aspdotnet: CreateRequest %s processing failed, file %s", r->uri, r->filename);
		status = HTTP_INTERNAL_SERVER_ERROR;
	}
...
static int asp_net_pre_config(apr_pool_t* pconf, apr_pool_t* plog,
	...
	if((apr_pool_userdata_get((void**)&global_conf, "mod_aspdotnet::global_conf", process) != APR_SUCCESS) || !global_conf) {
		...
		apr_pool_userdata_setn(global_conf, "mod_aspdotnet::global_conf", apr_pool_cleanup_null, process);
	}
	return APR_SUCCESS;
}
...
static int asp_net_post_config(apr_pool_t* pconf, apr_pool_t* plog,
	...
	ap_add_version_component(pconf, "mod_aspdotnet/" APW_STRINGIFY(APACHE_WEB_VER_MAJOR) "." APW_STRINGIFY(APACHE_WEB_VER_MINOR));
	...
	if(!global_conf->pCorRuntime || !global_conf->pHostFactory) {
		...
		if(FAILED(hr)) {
			ap_log_error(APLOG_MARK, APLOG_CRIT, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Failed to CoInitialize the threaded COM engine for .NET CLR interop!");
			return 1;
		}
		try {
			...
			ap_log_error(APLOG_MARK, APLOG_CRIT,
				APR_FROM_OS_ERROR(err.Error()), global_server, "mod_aspdotnet: Failed to start Asp.Net Apache.Web host factory");
			if(desc) {
				ap_log_error(APLOG_MARK, APLOG_CRIT, APR_FROM_OS_ERROR(err.Error()), global_server, "mod_aspdotnet: %s", desc);
			}
			return 1;
		} catch(HRESULT hr) {
			ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Failed to start Asp.Net Apache.Web host factory.");
			return 1;
		} catch(...) {
			ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: Failed to start Asp.Net Apache.Web host factory.");
			return 1;
		}
	}
	...
	for(item = apr_hash_first(ptemp, global_conf->hosts); item;
		...
		try {
			...
			ap_log_error(
				APLOG_MARK,
				APLOG_CRIT,
				APR_FROM_OS_ERROR(err.Error()),
				global_server, "mod_aspdotnet: Failed to create Host connector for %s mapped to %s",
				host->virtual_path,
				host->physical_path
			);
			ap_log_error(APLOG_MARK, APLOG_CRIT, APR_FROM_OS_ERROR(err.Error()), global_server, "mod_aspdotnet: %s", (char*)err.Description());
			return 1;
		} catch(HRESULT hr) {
			ap_log_error(APLOG_MARK, APLOG_ERR, APR_FROM_OS_ERROR(hr), global_server, "mod_aspdotnet: Failed to create Host for %s mapped to %s", host->virtual_path, host->physical_path);
			return 1;
		} catch(...) {
			ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: Failed to create Host for %s mapped to %s", host->virtual_path, host->physical_path);
			return 1;
		}
		ap_log_error(APLOG_MARK, APLOG_DEBUG | APLOG_NOERRNO, 0, global_server, "mod_aspdotnet: Sucessfully created Host for %s mapped to %s", host->virtual_path, host->physical_path);
	}
