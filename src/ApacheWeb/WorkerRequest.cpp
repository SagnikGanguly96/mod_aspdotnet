...
Apache::Web::WorkerRequest::WorkerRequest(UINT_PTR Req, Host^ ThisHost) : SimpleWorkerRequest(String::Empty, String::Empty, nullptr) {
	...
	filename = gcnew String(rr->filename ? rr->filename : "", 0, rr->filename ? strlen(rr->filename) : 0, utf8_encoding);
...
#ifdef _DEBUG
	String^ res = String::Concat(L"MapPath: ", path, L" mapped to ", mapped);
...
#pragma unmanaged
static apr_status_t InternalResponseFromMemory(request_rec* rr, const char* buf_data, int length) {
	conn_rec* c = rr->connection;
	apr_bucket_brigade* bb;
	apr_bucket* b;
...
void Apache::Web::WorkerRequest::SendResponseFromMemory(IntPtr data, int length) {
	if(!length) {
#ifdef _DEBUG
...
#ifdef _DEBUG
	String^ res = String::Concat(L"SendResponseFromMemory: passed ", (new Int32(length))->ToString(), L" byte buffer");
	LogRequestError(res, APLOG_DEBUG, 0);
#endif
...
#pragma unmanaged
static apr_status_t InternalResponseFromFile(request_rec* rr, HANDLE hv, __int64 offset, __int64 length) {
	conn_rec* c = rr->connection;
	apr_bucket_brigade* bb;
	apr_bucket* b;
	apr_file_t* fd;
...
#endif
		b = apr_bucket_file_create(fd, offset, (apr_size_t)fsize, rr->pool, c->bucket_alloc);
	APR_BRIGADE_INSERT_TAIL(bb, b);
	b = apr_bucket_flush_create(c->bucket_alloc);
	APR_BRIGADE_INSERT_TAIL(bb, b);
	return ap_pass_brigade(rr->output_filters, bb);
...
void Apache::Web::WorkerRequest::SendResponseFromFile(IntPtr handle, __int64 offset, __int64 length) {
	if(!length) {
#ifdef _DEBUG
...
#ifdef _DEBUG
	String^ res = String::Concat(L"SendResponseFromFile: passed ", (new Int64(length))->ToString(), L" byte request starting at offset ", (new Int64(offset))->ToString());
	LogRequestError(res, APLOG_DEBUG, 0);
#endif
...
#pragma unmanaged
static apr_status_t InternalReadEntityBody(request_rec* rr, char* curptr, apr_size_t size, int* bytes_read, int* overflow, int* seen_eos) {
	apr_bucket_brigade* bb;
	apr_status_t rv;
	*overflow = 0;
	*bytes_read = 0;
	...
	do {
		...
		if((rv = ap_get_brigade(rr->input_filters, bb, AP_MODE_READBYTES, APR_BLOCK_READ, size_one)) != APR_SUCCESS) {
			return rv;
		}
...
int Apache::Web::WorkerRequest::ReadEntityBody(array<unsigned char>^ buffer, int size) {
	if(buffer->Length < 1) {
#ifdef _DEBUG
	...
	apr_status_t rv = InternalReadEntityBody(rr, static_cast<char*>(startptr), (apr_size_t)size, &read, &overflow, eos);
	bytes_read += read;
	if(rv != APR_SUCCESS) {
		LogRequestError(L"ReadEntityBody: brigade read failure " L"reading the request body", APLOG_ERR, rv);
	}
	if(overflow) {
		LogRequestError(L"ReadEntityBody: brigade read overflow " L"reading the request body", APLOG_ERR, rv);
	}
#ifdef _DEBUG
	String^ res = String::Concat(L"ReadEntityBody: read ", (new Int32(read))->ToString(), L" bytes");
	LogRequestError(res, APLOG_DEBUG, 0);
#endif
	return read;
}
...
int Apache::Web::WorkerRequest::Handler(void) {
	...
	if((rv = ap_get_brigade(rr->input_filters, bb,
		...
		while(!APR_BRIGADE_EMPTY(bb)) {
			...
			rv = apr_bucket_read(bucket, &data, &len, APR_BLOCK_READ);
			if(rv != APR_SUCCESS) {
				LogRequestError(L"Handler: brigade read failure prereading " L"the request body", APLOG_ERR, rv);
			}
			if(len > size) {
				len = size;
				LogRequestError(L"Handler: brigade read overflow prereading " L"the request body", APLOG_ERR, 0);
			}
			if(size) {
				memcpy(curptr, data, len);
				bytes_read += len;
		...
		apr_brigade_cleanup(bb);
#ifdef _DEBUG
		String^ res = String::Concat(L"Handler: preread ", (new Int64(bytes_read))->ToString(), L" request body bytes");
		LogRequestError(res, APLOG_DEBUG, 0);
#endif
	} else {
