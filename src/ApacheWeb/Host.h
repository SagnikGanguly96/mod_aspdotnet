	...
	namespace Web {
		...
		public:
			...
			virtual Object^ InitializeLifetimeService() override {
				ILease^ lease = safe_cast<ILease^> (MarshalByRefObject::InitializeLifetimeService());
			...
			void LogRequestError(String^ msg, int loglevel, apr_status_t rv, request_rec* rr) {
				array<unsigned char>^ avalue = utf8_encoding->GetBytes(msg);
				pin_ptr<unsigned char> msg_text = &avalue[0];
				ap_log_rerror("Apache.Web", 0, thisModuleIndex, loglevel | (rv ? 0 : APLOG_NOERRNO), rv, rr, "mod_aspdotnet: %s", (char*)msg_text, NULL);
			}
			...
			int HandleRequest(UINT_PTR ReqRecordPtr) {
				try {
					WorkerRequest^ request = gcnew WorkerRequest(ReqRecordPtr, this);
					...
					LogRequestError(e->ToString(), APLOG_ERR, 0, (request_rec*)ReqRecordPtr);
			...
			String^ MapPath(String^ path, request_rec* rr) {
				/* Relative to this host's Physical path.
				The function returns NULL if the given URI-path is outside of the Virtual path of this host. */
				String^ buildpath;
				...
				if(testpath[0] == L'/') {
					if(String::Compare(testpath, 0, virtualPath, 0, virtualPath->Length, true, nullCulture)) {
						String^ res = String::Concat(L"MapPath: could not map ", path, L" within ", virtualPath);
						LogRequestError(res, APLOG_DEBUG, 0, rr);

						return nullptr;
					} else {
						if(testpath->Length > virtualPath->Length) {
							buildpath = String::Concat(physicalPath, buildpath->Substring(virtualPath->Length));
						} else {
							buildpath = physicalPath;
						}
					}
				} else {
					buildpath = String::Concat(physicalPath, L"\\", buildpath->Substring(virtualPath->Length));
				}
